...
def show_movie(id)
  #check if movie already exists in own movie db and if it does go to erb :movie
  @movie = Movie.find_by(imdbid:id)
  if @movie.nil?
    movie_request = "http://www.omdbapi.com/?apikey=2f6435d9&i=#{id}"
    @result = HTTParty.get(movie_request)
    @movie = Movie.new(title: @result['Title'], director: @result['Director'], actors: @result['Actors'], plot: @result['Plot'], poster: @result['Poster'], imdbid: @result['imdbID'])
    @movie.save
  end
  erb :movie
end
